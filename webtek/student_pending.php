...
<div class="container">
    ...
    <nav class="navbar navbar-inverse">
        <div class="container-fluid">
            ...
            <ul class="nav navbar-nav">
                ...
                <li class="active"><a href="student_pending.php">Requests</a></li>
    ...
    <div class="row justify-content-center">
        <table class="table">
            ...
            <?php
            $id = $_SESSION['idno'];
            $sql = "SELECT petitions.subject_id AS si, accounts.firstname AS fn, accounts.lastname AS ln, subjects.minimum AS mini, subjects.slots AS slots, petitions.status AS status FROM petitions JOIN subjects ON petitions.subject_id = subjects.subject_id JOIN accounts ON petitions.student_id = accounts.student_id WHERE petitions.status = 'Pending' AND accounts.student_id = '$id'";
            $res = $conn->query($sql);
            while ($row = $res->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . strtoupper($row['fn']) . " " . strtoupper($row['fn']) . "</td>";
                echo "<td>" . $row['mini'] . "</td>";
                echo "<td>" .  $row['slots'] . "</td>";
                echo "<td>" .  $row['status'] . "</td>";
                echo "  <td>
                        <a href='view_students1.php?num=" . $row['si'] .  "' class='btn btn-info'>View Students</a>
                        <a href='join_subject.php?num=" . $row['si'] . "&idno=" . $id . "' class='btn btn-danger'>Cancel</a>
                    </td>
                </tr>";
            }
            ?>
